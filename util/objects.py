...
class GoslingAgent(BaseAgent):
    ...
    def initialize_agent(self):
        ...
        # Store the intent in text to print on screen for debugging
        self.debug_text = ''
        # list of debug line objects that can be drawn on screen
        self.debug_lines = []
    ...
    def is_in_front_of_ball(self):
        ...
        if me_to_goal < (ball_to_goal + 800) and me_to_goal > 200:
            return True
        return False

    def print_debug(self):
        white = self.renderer.white()
        self.renderer.draw_string_2d(10, 200, 3, 3, self.debug_text, white)

    def add_debug_line(self, name, vec1, vec2, color=[255, 255, 255]):
        duplicates = [line for line in self.debug_lines if line.name == name]
        if len(duplicates) > 0:
            return
    ...
    def remove_debug_line(self, name):
        self.debug_lines = [
            line for line in self.debug_lines if line.name != name]
